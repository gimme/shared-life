...
public class NeoForgeMod {
    ...
    public void onServerStarting(ServerAboutToStartEvent event) {
        var playerConfig = new PlayerConfig(new NeoForgeConfig());
        var sharedLife = new SharedLife(playerConfig, new NeoForgeThirstPlugin());

        NeoForge.EVENT_BUS.register(new ServerListener(new ServerHandler(sharedLife)));
        NeoForge.EVENT_BUS.register(new PlayerListener(new PlayerHandler(sharedLife, playerConfig)));
    }
}
